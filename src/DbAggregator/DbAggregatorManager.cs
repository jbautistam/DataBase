...
namespace Bau.Libraries.DbAggregator
{
	...
	public class DbAggregatorManager
	{
		...
		{
			Providers.Add(connection.Key, GetDataProvider(connection));
		}
		...
		{
			bool IsConnection(ConnectionModel conn, ConnectionModel.DataBaseType type)
			{
				return conn.Type.EqualsIgnoreCase(type.ToString());
			}

			// Obtiene el proveedor
			if (connection != null)
			{
				if (IsConnection(connection, ConnectionModel.DataBaseType.SqLite))
					return GetSqLiteDataProvider(connection.Key, connection.Parameters);
				else if (IsConnection(connection, ConnectionModel.DataBaseType.SqlServer))
					return GetSqlServerDataProvider(connection.Key, connection.Parameters);
				else if (IsConnection(connection, ConnectionModel.DataBaseType.Odbc))
					return GetOdbcDataProvider(connection.Key, connection.Parameters);
				else if (IsConnection(connection, ConnectionModel.DataBaseType.PostgreSql))
					return GetPostgreSqlDataProvider(connection.Key, connection.Parameters);
				else if (IsConnection(connection, ConnectionModel.DataBaseType.MySql))
					return GetMySqlDataProvider(connection.Key, connection.Parameters);
				else if (IsConnection(connection, ConnectionModel.DataBaseType.Spark))
