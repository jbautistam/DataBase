	...
	public class ProviderModel
	{
		...
		public int Execute(CommandModel command)
		{
			int result;
				...
				Provider.Open();
				// Ejecuta el comando
				result = Provider.Execute(command.Sql, GetParametersDb(command.Parameters), CommandType.Text);
				// Cierra la conexión
				Provider.Close();
		...
		/// </summary>
		public SchemaDbModel LoadSchema()
		{
			return Task.Run(async () => await LoadSchemaAsync(TimeSpan.FromMinutes(2), CancellationToken.None)).Result;
		}

		/// <summary>
		///		Carga el esquema de base de datos
		/// </summary>
		public async Task<SchemaDbModel> LoadSchemaAsync(TimeSpan timeout, CancellationToken cancellationToken)
		{
			return await Provider.GetSchemaAsync(timeout, cancellationToken);
		}
