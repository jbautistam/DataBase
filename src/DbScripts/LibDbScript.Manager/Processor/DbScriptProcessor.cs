	...
	{   
		...
		internal void Process()
		{
			...
			foreach (KeyValuePair<string, object> keyValue in Generator.Parameters)
				Context.Actual.VariablesTable.Add(keyValue.Key, keyValue.Value);
		...
		private void ExecuteFor(SentenceFor sentence)
		{
					 ...
					 index += sentence.Step)
			{
				...
				Context.Actual.VariablesTable.Add(sentence.Variable, index);
				// Ejecuta las sentencias
				Execute(sentence.Sentences);
				// Elimina el contexto
				Context.Pop();
		...
		{
			IDataProvider provider = Generator.DataProviderManager.GetDataProvider(sentence.ProviderKey);
		...
		{
			IDataProvider provider = Generator.DataProviderManager.GetDataProvider(providerKey);
